	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div class="panel-body">
				...
				$qry_skck = mysqli_query($conn, "SELECT * from tbl_skck where status='Belum Terverifikasi'");
				if (isset($_POST['sudah'])) {
					$title = "Sudah Terverifikasi";
					$qry_skck = mysqli_query($conn, "SELECT * from tbl_skck where status='Terverifikasi'");
					...
					<table class="table table-bordered" style="margin-top:20px;">
						...
						<?php while ($data = mysqli_fetch_array($qry_skck)) { ?>
							<tr>
								...
								<td><a href="?module=edit_skck&id=<?php echo $data['id_skck']; ?>"><span class="glyphicon glyphicon-pencil"></span></a> <a href="hapus_skck.php?id=<?php echo $data['id_skck']; ?>"><span class="glyphicon glyphicon-trash"></span></a>
									<a href="print_skck.php?id=<?php echo $data['id_skck']; ?>" target="_blank"><span class="glyphicon glyphicon-print"></span></a>
									<?php if ($title === "Sudah Terverifikasi") {
										$pesan = "Halo" . $data['nama'] . "%0APermintaan Dengan %0ATujuan :". $data['keperluan'] . "%0ADengan Keterangan SKCK :". $data['keterangan'] . "%0ABERHASIL DIVERIFIKASI %0A%0ASilahkan Datang Ke SPKT Kepolisian Resor Pati Untuk Mencetak Lampiran SKCK https://goo.gl/maps/QUGbbhVXAKvcFmPRA" ;
									...
									<?php } else {
										$pesan = "Halo" . $data['nama'] . "%0APermintaan Dengan %0ATujuan :" . $data['keperluan'] . "%0ADengan Keterangan SKCK :" . $data['keterangan'] . "%0ABELUM DIVERIFIKASI %0A%0ASilahkan Untuk Melengkapi Berkas Anda";
										$url = 'https://wa.me/62' . $data['no_telp'] . '?text=' . $pesan; ?>
										<a href="<?php echo $url; ?>" target="_blank"><span class="glyphicon glyphicon-earphone"></span></a>
									<?php } ?>
								</td>
